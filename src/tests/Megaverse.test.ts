...
describe('Phase 2 Megaverse', () => {
    ...
    beforeEach(() => {
        ...
        // Initialize Services
        AstralObjectFactory.initialize();
        ShapeDrawer.initialize(LocalMegaverse.initialize(30, 30));

        megaverse = LocalMegaverse.getInstance();
        shapeDrawer = ShapeDrawer.getInstance();
        astralObjectFactory = AstralObjectFactory.getInstance();
        cleanMap = {
            goal: Array.from({ length: 30 }, (_, row) =>
                Array.from({ length: 30 }, (_, column) => new Space({ x: row, y: column }).toString())
            )
        };
    });


    describe('Phase 2: getGoalMap', () => {
        it('should return the goal map from Phase 2', async () => {
            const mockGoalMap: MegaverseSpace = goalmap2;
            // This will mock the goal map from the CrossMint API in the LocalMegaverse
            megaverse.resetMegaverse(30, 30);
            megaverse.setGoalMap(mockGoalMap);

            await shapeDrawer.drawCrossMintLogo();
            const goalMap = await megaverse.getGoalMap();
            ...
            // Clean up the map
            await shapeDrawer.clearCrossMintLogo();
            const cleanGoalMap = await megaverse.getGoalMap();
            expect(_.isEqual(cleanGoalMap, cleanMap)).toBeTruthy();
